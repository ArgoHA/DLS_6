...
def build_model(n_outputs: int, device: str, layers_to_train: int) -> nn.Module:
    model = timm.create_model(
        model_name="efficientnet_b3.ra2_in1k", pretrained=True, num_classes=n_outputs
    )
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        save_metrics({}, metrics, np.mean(losses), path_to_save=None)
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg["use_scheduler"]:
        scheduler = OneCycleLR(
            ...
            pct_start=cfg.cycler_pct_start,
        )
